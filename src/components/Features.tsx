...
const features = [
  {
    ...
    detail: (
      <div className="mt-5 grid grid-cols-2 gap-px bg-border font-mono text-[11px]">
      ...
      <div className="mt-5">
        <div className="flex flex-wrap gap-px mb-3">
          {["Slack", "GitHub", "Postgres", "Notion", "Gmail", "Stripe"].map((t) => (
            <span key={t} className="font-mono text-[11px] bg-surface-elevated text-heading px-3 py-1.5 border border-border">{t}</span>
          ))}
          <span className="font-mono text-[11px] text-body px-3 py-1.5">+38 more</span>
        </div>
        <p className="font-mono text-[10px] text-body uppercase tracking-widest">Permissions managed centrally · Zero credential sprawl</p>
      </div>
    ),
  },
  {
    title: "One click deploy",
    desc: "One click. Your LangGraph agent is live. Olympus manages provisioning, versioning, and execution behind the scenes.",
    detail: (
      <div className="mt-5 bg-surface-elevated p-4 font-mono text-[11px] border border-border">
        <p className="text-accent">$ olympus deploy</p>
  ...
  },
  {
    ...
    detail: (
      <div className="mt-5 space-y-px font-mono text-[11px]">
        {[
          ...
          <div key={i} className="flex items-center gap-3 bg-surface-elevated px-4 py-2 border border-border">
    ...
    detail: (
      <div className="mt-5 space-y-px font-mono text-[11px]">
        {[
          ...
          <div key={r.task} className="flex justify-between bg-surface-elevated px-4 py-2.5 border border-border">
  ...
  <section id="features" className="py-24 section-divider">
    <div className="container max-w-5xl">
      <motion.h2
        ...
        viewport={{ once: true }}
        className="text-3xl sm:text-4xl md:text-5xl font-bold text-heading text-center mb-16 tracking-tight"
      >
        Features
      </motion.h2>
      ...
      <div className="grid md:grid-cols-2 gap-px bg-border mb-px">
        {features.slice(0, 2).map((f, i) => (
          <motion.div
            ...
            className="bg-card p-7 hover:bg-surface-elevated transition-colors"
          >
            <h3 className="text-sm font-semibold text-heading mb-2 uppercase tracking-wide">{f.title}</h3>
